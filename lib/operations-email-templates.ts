...
import {
  ...
  type OperationsTemplateStatus,
  type OperationsTemplateAudience,
...
/**
 * Operations Email Template Management service — mirrors the HR template
 * manager (stable Template ID OPET-0001, machine key, category/audience,
 * plain-text alternative, Draft/Active/Inactive/Archived lifecycle, versioning
 * and usage analytics), scoped to the operations_email_templates table.
 */
...
export async function ensureOperationsEmailTemplateSchema() {
  if (schemaEnsured) return

  await query(
    `CREATE TABLE IF NOT EXISTS operations_email_templates (
  ...
  await ensureColumn("operations_email_templates", "template_uid", "VARCHAR(40) NULL")
  await ensureColumn("operations_email_templates", "template_key", "VARCHAR(80) NULL")
  await ensureColumn("operations_email_templates", "description", "VARCHAR(500) NULL")
  await ensureColumn("operations_email_templates", "category", "VARCHAR(60) NOT NULL DEFAULT 'General'")
  await ensureColumn("operations_email_templates", "audience", "VARCHAR(40) NOT NULL DEFAULT 'Client'")
  await ensureColumn("operations_email_templates", "event_key", "VARCHAR(60) NULL")
  await ensureColumn("operations_email_templates", "status", "VARCHAR(20) NOT NULL DEFAULT 'Draft'")
  await ensureColumn("operations_email_templates", "body_text", "LONGTEXT NULL")
  await ensureColumn("operations_email_templates", "version", "INT UNSIGNED NOT NULL DEFAULT 1")
  await ensureColumn("operations_email_templates", "usage_count", "INT UNSIGNED NOT NULL DEFAULT 0")
  await ensureColumn("operations_email_templates", "last_used_at", "DATETIME NULL")
  await ensureColumn("operations_email_templates", "attachment_pathname", "VARCHAR(255) NULL")
  await ensureColumn("operations_email_templates", "attachment_name", "VARCHAR(255) NULL")
  await ensureColumn("operations_email_templates", "attachment_type", "VARCHAR(150) NULL")
  await ensureColumn("operations_email_templates", "attachment_size", "INT UNSIGNED NULL")
  await ensureColumn("operations_email_templates", "created_by", "BIGINT UNSIGNED NULL")
  await ensureColumn("operations_email_templates", "updated_by", "BIGINT UNSIGNED NULL")
  await ensureColumn(
    "operations_email_templates",
    "created_at",
    "TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP",
  )

  await query(
    `ALTER TABLE operations_email_templates MODIFY COLUMN status
     ENUM('Draft','Active','Inactive','Archived') NOT NULL DEFAULT 'Draft'`,
  )

  await ensureIndex(
    "operations_email_templates",
    "uq_operations_email_templates_uid",
    "UNIQUE KEY uq_operations_email_templates_uid (template_uid)",
  )
  await ensureIndex(
    "operations_email_templates",
    "uq_operations_email_templates_key",
    "UNIQUE KEY uq_operations_email_templates_key (template_key)",
  )
  await ensureIndex(
    "operations_email_templates",
    "idx_operations_email_templates_status",
    "KEY idx_operations_email_templates_status (status)",
  )

  await query(
    `CREATE TABLE IF NOT EXISTS operations_email_template_versions (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
      ...
      INDEX idx_operations_email_template_versions_tpl (template_id, version)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4`,
  )

  const legacy = await query<any[]>(
    "SELECT id FROM operations_email_templates WHERE template_uid IS NULL OR template_uid = ''",
  )
  for (const row of legacy) {
    await query("UPDATE operations_email_templates SET template_uid = ? WHERE id = ?", [
...
async function generateTemplateUid(): Promise<string> {
  return nextRecordId("OPET", { digits: 4, allowCustom: true }) // e.g. OPET-0001
}
...
const mapRow = (r: any): OperationsEmailTemplate => ({
  id: Number(r.id),
  ...
  status: (r.status ?? "Draft") as OperationsTemplateStatus,
...
export async function listTemplates(
  ...
  await ensureOperationsEmailTemplateSchema()
  const where: string[] = []
  const args: any[] = []
  ...
  const rows = await query<any[]>(
    `SELECT * FROM operations_email_templates
...
export async function getTemplate(id: number): Promise<OperationsEmailTemplate | null> {
  await ensureOperationsEmailTemplateSchema()
  const rows = await query<any[]>("SELECT * FROM operations_email_templates WHERE id = ? LIMIT 1", [id])
...
function validate(input: TemplateInput): string | null {
  ...
  if (input.status && !OPERATIONS_TEMPLATE_STATUSES.includes(input.status)) return "Invalid status"
  return null
}

async function keyTaken(key: string, exceptId?: number): Promise<boolean> {
  const rows = await query<any[]>(
    `SELECT id FROM operations_email_templates WHERE template_key = ? ${exceptId ? "AND id <> ?" : ""} LIMIT 1`,
...
export async function createTemplate(
  ...
  const res = await query<any>(
    `INSERT INTO operations_email_templates
    ...
    [
      ...
      (input.audience || "Client").trim() || "Client",
...
export async function updateTemplate(
  ...
  await query(
    `UPDATE operations_email_templates SET
...
export async function deleteTemplate(id: number, opts: { hard?: boolean } = {}): Promise<TemplateResult> {
  ...
  if (opts.hard && existing.usage_count === 0) {
    await query("DELETE FROM operations_email_templates WHERE id = ?", [id])
  } else {
    await query("UPDATE operations_email_templates SET status = 'Archived' WHERE id = ?", [id])
  }
  return { ok: true, id, template_uid: existing.template_uid || "" }
}
...
export async function recordTemplateUsage(id: number): Promise<void> {
  try {
    await query(
      "UPDATE operations_email_templates SET usage_count = usage_count + 1, last_used_at = NOW() WHERE id = ?",
      [id],
    )
  } catch {
...
async function snapshotVersion(
  ...
  try {
    await query(
      `INSERT INTO operations_email_template_versions
...
export async function listVersions(templateId: number) {
  ...
  return query<any[]>(
    `SELECT id, version, name, subject, status, changed_by, created_at
     FROM operations_email_template_versions WHERE template_id = ? ORDER BY version DESC`,
    [templateId],
  )
}
