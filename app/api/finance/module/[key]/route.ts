...
import { createFinanceHandlers } from "@/lib/finance-crud"

// One route serves every config-driven Finance module. The [key] segment
// (e.g. "expenses", "gst-filing") selects the ModuleConfig, and the shared
// CRUD factory handles auto IDs, server-side calculations and the
// created_at / updated_at system columns for that module's dedicated table.

type Handlers = ReturnType<typeof createFinanceHandlers>
const cache = new Map<string, Handlers>()

function getHandlers(key: string): Handlers | null {
  if (cache.has(key)) return cache.get(key)!
  try {
    const handlers = createFinanceHandlers(key)
...
async function withHandlers(
  ...
  if (!handlers) return NextResponse.json({ error: "Unknown finance module" }, { status: 404 })
  return run(handlers)
}
