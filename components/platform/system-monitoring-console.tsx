...
export function SystemMonitoringConsole() {
  ...
  useEffect(() => {
    ...
    if (id && /^[1-9]\d*$/.test(id)) { setFilters((current) => ({ ...current, tenantId: id })); setQueryFilters((current) => ({ ...current, tenantId: id })); setView("logs") }
  }, [])

  const load = useCallback(async () => {
    setLoading(true); setError("")
    const params = new URLSearchParams({ hours: String(hours) })
    if (view === "logs") { Object.entries(queryFilters).forEach(([key, value]) => { if (value) params.set(key, value) }); if (cursor) params.set("cursor", String(cursor)) }
  ...
  }, [view, hours, queryFilters, cursor])
  ...
  return <main className="space-y-6 p-6 text-foreground">
    ...
    {detail?.kind === "incidents" && <section className="rounded-xl border p-4"><h3 className="font-semibold">Recent occurrences</h3>{detail.logs?.length ? detail.logs.map((log: any) => <button key={log.id} className="block w-full border-b py-2 text-left text-sm hover:text-primary" onClick={() => void openDetail("logs", log.id)}>{stamp(log.created_at)} · {log.severity} · {log.message} · {log.request_id || "No request ID"}</button>) : <p className="py-3 text-muted-foreground">No retained log occurrences.</p>}</section>}
  </main>
}
