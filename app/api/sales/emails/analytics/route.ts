...
/**
 * Sales email analytics — the sales counterpart of the HR email analytics
 * route. Aggregates the single `sales_emails` table into the numbers the
 * Analytics tab renders: delivery funnel, open rate, manual-vs-automated split,
 * per-category and per-status breakdowns, top automated flows, and a daily
 * volume trend. Read-only; gated on the send-emails permission.
 */
export async function GET(request: NextRequest) {
  ...
  try {
      ...
      query<any[]>(
        `SELECT source_module AS module, COUNT(*) AS count,
         ...
         WHERE email_type = 'Automated'
           AND sent_at >= (NOW() - INTERVAL ${since})
    ...
    console.log("[v0] sales email analytics failed:", (error as Error).message)
