...
export function SalesEmailAutomationSettings() {
  const { data, mutate, isLoading } = useSWR<{ events: EventConfig[]; canManage: boolean }>(
    "/api/sales/emails/automations",
    fetcher,
  )
  ...
  async function patch(key: string, body: Record<string, unknown>) {
    ...
    try {
      const res = await fetch("/api/sales/emails/automations", {
        method: "PATCH",
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          Control which sales workflow events send an automated email and the template they use.
          Every event is off by default.
        </p>
        {!canManage && (
