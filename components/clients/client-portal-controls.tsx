...
/**
 * All admin-side client-portal controls for a single client: resource grants,
 * login accounts, and shared records. Rendered both inside the per-client
 * "Manage portal" dialog and on the dedicated Client Portal module page.
 * `active` gates data fetching so the SWR calls only fire when the surface is
 * actually visible (dialog open / client selected).
 */
export function ClientPortalControls({
  client,
  active,
}: {
  ...
  const access = useSWR<{ resources: PortalResource[] }>(
    active && clientId ? `/api/admin/portal/access?clientId=${clientId}` : null,
    fetcher,
  )
  const usersReq = useSWR<{ users: PortalUser[] }>(
    active && clientId ? "/api/admin/portal/users" : null,
    fetcher,
  )
  const itemsReq = useSWR<{ items: SharedItem[] }>(
    active && clientId ? `/api/admin/portal/items?clientId=${clientId}` : null,
    fetcher,
  )
