...
import { FinanceEmailTemplateDialog } from "@/components/finance/finance-email-template-dialog"

type ApiResponse = { templates: FinanceEmailTemplate[]; canManage: boolean }
...
export function FinanceEmailTemplatesClient() {
  const [search, setSearch] = useState("")
  const [status, setStatus] = useState("all")
  const [category, setCategory] = useState("all")
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editing, setEditing] = useState<FinanceEmailTemplate | null>(null)

  const { data, isLoading, mutate } = useSWR<ApiResponse>(
    "/api/finance/email-templates?includeArchived=1",
    fetcher,
  )
  ...
  function openEdit(t: FinanceEmailTemplate) {
    setEditing(t)
    setDialogOpen(true)
  }

  async function setStatusFor(t: FinanceEmailTemplate, next: string) {
    const res = await fetch(`/api/finance/email-templates/${t.id}`, {
      method: "PATCH",
  ...
  async function duplicate(t: FinanceEmailTemplate) {
    const res = await fetch(`/api/finance/email-templates/${t.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function remove(t: FinanceEmailTemplate) {
    const res = await fetch(`/api/finance/email-templates/${t.id}`, { method: "DELETE" })
    if (res.ok) {
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {templates.length === 0
                        ? "Create your first finance email template to get started."
