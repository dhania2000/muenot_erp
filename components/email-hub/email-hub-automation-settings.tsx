...
type Template = { id: number; name: string; status?: string }
...
export function EmailHubAutomationSettings({ config }: { config: EmailHubUiConfig }) {
  const { data, mutate, isLoading } = useSWR<{ events: EventConfig[]; canManage: boolean }>(
    `${config.apiBase}/automations`,
    fetcher,
  )
  const canManage = data?.canManage ?? false
  const { data: templateData } = useSWR<{ templates: Template[] }>(config.templatesUrl, fetcher)
    ...
    mutate(
      (prev) =>
        prev
          ? {
              ...prev,
              events: prev.events.map((e) => (e.key === key ? { ...e, ...body } : e)),
            }
          : prev,
      false,
    )
  ...
  if (isLoading && !data) {
    return (
      <p className="py-10 text-center text-sm text-muted-foreground">Loading automation rules…</p>
    )
  }
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          Control which workflow events send an automated email and the template they use.
        </p>
        {!canManage && (
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <Toggle
                      checked={ev.enabled}
                      disabled={!canManage || savingKey === ev.key}
                      onChange={(next) => patch(ev.key, { enabled: next })}
                      label={`Enable ${ev.label}`}
                    />
                    <span className="font-medium">{ev.label}</span>
                  </div>
                  <p className="mt-1 truncate text-xs text-muted-foreground">{ev.description}</p>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 md:justify-end">
                  <Select
                    ...
                    onValueChange={(v) =>
                      patch(ev.key, { template_id: v === NO_TEMPLATE ? null : Number(v) })
                    }
