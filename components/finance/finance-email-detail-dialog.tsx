...
export function FinanceEmailDetailDialog({
  emailId,
  onClose,
  onChanged,
}: {
  ...
  const { data, mutate, isLoading } = useSWR<{ email: any }>(
    emailId ? `/api/finance/emails/${emailId}` : null,
    fetcher,
  )
  ...
  async function act(action: "resend" | "cancel" | "send") {
    ...
    setBusy(true)
    try {
      const res = await fetch(`/api/finance/emails/${emailId}`, {
        method: "PATCH",
      ...
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {email && <FinanceEmailStatusBadge status={email.status} />}
          </DialogTitle>
        </DialogHeader>
        ...
        {isLoading || !email ? (
          ...
          <div className="space-y-5">
            <dl className="grid grid-cols-2 gap-4">
              <Field
                label="To"
                value={`${email.to_name ? `${email.to_name} · ` : ""}${email.to_email}`}
              />
              ...
              <Field
                ...
                value={new Date(email.scheduled_at || email.sent_at || email.created_at).toLocaleString()}
              />
            ...
            <div className="space-y-1.5">
              ...
              <div
                ...
                // Rendering stored finance email HTML authored internally.
