...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const to = (body.to || inv.freelancer_email || "").trim()
  if (!to) {
    return NextResponse.json(
      { error: "No recipient email. Add the freelancer's email on the invoice or enter one." },
      { status: 400 },
    )
  }
  ...
  const subject = (body.subject || `Invoice ${invoiceNo} from ${company.name}`).trim()
  const message = (body.message || "").trim()

  const defaultMessage =
    `Dear ${inv.freelancer_name || "Freelancer"},<br/><br/>` +
  ...
  try {
    ...
    await query(
      `UPDATE freelance_invoices SET invoice_last_sent_at=NOW(), invoice_last_sent_to=? WHERE id=?`,
      [to, inv.id],
    )
  } catch (err) {
    if (emailId != null) {
    ...
    console.log("[v0] Freelance invoice email send failed:", (err as any)?.message)
