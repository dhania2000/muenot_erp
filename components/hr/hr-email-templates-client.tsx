...
export function HrEmailTemplatesClient() {
  ...
  const { data, isLoading, mutate } = useSWR<ApiResponse>(
    "/api/hr/email-templates?includeArchived=1",
    fetcher,
  )
  ...
  function openEdit(t: HrEmailTemplate) {
    setEditing(t)
    setDialogOpen(true)
  }

  async function setStatusFor(t: HrEmailTemplate, next: string) {
    const res = await fetch(`/api/hr/email-templates/${t.id}`, {
      method: "PATCH",
  ...
  async function duplicate(t: HrEmailTemplate) {
    const res = await fetch(`/api/hr/email-templates/${t.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function remove(t: HrEmailTemplate) {
    const res = await fetch(`/api/hr/email-templates/${t.id}`, { method: "DELETE" })
    if (res.ok) {
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {templates.length === 0
                        ? "Create your first HR email template to get started."
