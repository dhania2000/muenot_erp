...
import { FINANCE_EMAIL_CATEGORIES, FINANCE_EMAIL_STATUSES } from "@/lib/finance-email-shared"
import { FinanceEmailComposer } from "@/components/finance/finance-email-composer"
import { FinanceEmailDetailDialog } from "@/components/finance/finance-email-detail-dialog"
...
export function FinanceEmailHub() {
  ...
  const { data, mutate, isLoading } = useSWR<EmailsResponse>(
    `/api/finance/emails?${params.toString()}`,
    fetcher,
  ...
  return (
    <main className="space-y-6 p-6">
      <header className="space-y-1">
        ...
        <h1 className="text-3xl font-semibold">Finance Email Hub</h1>
      ...
      {data && !data.configured && (
        <p className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-300">
          Finance email transport is not configured yet. Emails can be drafted and scheduled but
          will fail to send until finance SMTP / Gmail is set up in settings.
        </p>
      )}
      ...
      <Tabs value={tab} onValueChange={setTab} className="space-y-4">
                  ...
                  {isLoading && emails.length === 0 ? (
                    ...
                    emails.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <FinanceEmailStatusBadge status={row.status} />
                        </td>
        ...
        <TabsContent value="analytics">
          <FinanceEmailAnalytics />
        </TabsContent>

        <TabsContent value="automation">
          <FinanceEmailAutomationSettings />
        </TabsContent>
      </Tabs>

      <FinanceEmailDetailDialog
        emailId={detailId}
