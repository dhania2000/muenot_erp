 ...
 *
 * Owns the full lifecycle of reusable HR email templates: stable Template ID
 * (HRET-0001), machine key, category/audience, optional automation-event
 * mapping, plain-text alternative, Draft/Active/Inactive/Archived status,
 * versioning and usage analytics. Every automated and manual HR email that
...
export async function ensureHrEmailTemplateSchema() {
  ...
  await ensureColumn("hr_email_templates", "template_uid", "VARCHAR(40) NULL")
  await ensureColumn("hr_email_templates", "template_key", "VARCHAR(80) NULL")
  await ensureColumn("hr_email_templates", "description", "VARCHAR(500) NULL")
  await ensureColumn("hr_email_templates", "category", "VARCHAR(60) NOT NULL DEFAULT 'General'")
  await ensureColumn("hr_email_templates", "audience", "VARCHAR(40) NOT NULL DEFAULT 'Employee'")
  await ensureColumn("hr_email_templates", "event_key", "VARCHAR(60) NULL")
  await ensureColumn("hr_email_templates", "body_text", "LONGTEXT NULL")
  await ensureColumn("hr_email_templates", "version", "INT UNSIGNED NOT NULL DEFAULT 1")
  await ensureColumn("hr_email_templates", "usage_count", "INT UNSIGNED NOT NULL DEFAULT 0")
  await ensureColumn("hr_email_templates", "last_used_at", "DATETIME NULL")
  await ensureColumn("hr_email_templates", "attachment_pathname", "VARCHAR(255) NULL")
  await ensureColumn("hr_email_templates", "attachment_name", "VARCHAR(255) NULL")
  await ensureColumn("hr_email_templates", "attachment_type", "VARCHAR(150) NULL")
  await ensureColumn("hr_email_templates", "attachment_size", "INT UNSIGNED NULL")
  await ensureColumn("hr_email_templates", "created_by", "BIGINT UNSIGNED NULL")
  await ensureColumn("hr_email_templates", "updated_by", "BIGINT UNSIGNED NULL")
  await ensureColumn(
    "hr_email_templates",
    "created_at",
  ...
  // Expand status into the full lifecycle (safe to run repeatedly).
  await query(
    `ALTER TABLE hr_email_templates MODIFY COLUMN status
     ENUM('Draft','Active','Inactive','Archived') NOT NULL DEFAULT 'Draft'`,
  )

  await ensureIndex(
    "hr_email_templates",
    "uq_hr_email_templates_uid",
    "UNIQUE KEY uq_hr_email_templates_uid (template_uid)",
  )
  await ensureIndex(
    "hr_email_templates",
    "uq_hr_email_templates_key",
    "UNIQUE KEY uq_hr_email_templates_key (template_key)",
  )
  await ensureIndex(
    "hr_email_templates",
    "idx_hr_email_templates_status",
    "KEY idx_hr_email_templates_status (status)",
  )
  ...
  const legacy = await query<any[]>(
    "SELECT id FROM hr_email_templates WHERE template_uid IS NULL OR template_uid = ''",
  )
  for (const row of legacy) {
    await query("UPDATE hr_email_templates SET template_uid = ? WHERE id = ?", [
...
async function generateTemplateUid(): Promise<string> {
  const raw = await nextRecordId("HRET", { digits: 4, allowCustom: true })
  return raw // e.g. HRET-0001
}
...
type MapRow = (row: any) => HrEmailTemplate
const mapRow: MapRow = (r) => ({
  id: Number(r.id),
  ...
  status: (r.status ?? "Draft") as HrTemplateStatus,
...
export async function listTemplates(opts: {
...
} = {}): Promise<HrEmailTemplate[]> {
  await ensureHrEmailTemplateSchema()
  const where: string[] = []
  const args: any[] = []
  ...
  const rows = await query<any[]>(
    `SELECT * FROM hr_email_templates
...
export async function getTemplate(id: number): Promise<HrEmailTemplate | null> {
  await ensureHrEmailTemplateSchema()
  const rows = await query<any[]>("SELECT * FROM hr_email_templates WHERE id = ? LIMIT 1", [id])
...
function validate(input: TemplateInput): string | null {
  ...
  if (input.status && !HR_TEMPLATE_STATUSES.includes(input.status)) return "Invalid status"
  return null
}

async function keyTaken(key: string, exceptId?: number): Promise<boolean> {
  const rows = await query<any[]>(
    `SELECT id FROM hr_email_templates WHERE template_key = ? ${exceptId ? "AND id <> ?" : ""} LIMIT 1`,
...
export async function createTemplate(
  ...
  const res = await query<any>(
    `INSERT INTO hr_email_templates
    ...
    [
      ...
      (input.audience || "Employee").trim() || "Employee",
...
export async function deleteTemplate(id: number, opts: { hard?: boolean } = {}): Promise<TemplateResult> {
  ...
  // Never orphan an automation mapping.
  const mapped = await query<any[]>(
    "SELECT event_key FROM hr_email_automations WHERE template_id = ? LIMIT 1",
    [id],
  ).catch(() => [])
  if (mapped.length > 0) {
    return {
      ok: false,
      error: "This template is mapped to an automation event. Unmap it before deleting.",
      code: 409,
    }
  }
  ...
  if (opts.hard && existing.usage_count === 0) {
    ...
    await query("UPDATE hr_email_templates SET status = 'Archived' WHERE id = ?", [id])
  }
  return { ok: true, id, template_uid: existing.template_uid || "" }
}
  ...
  return createTemplate(
    {
      ...
      event_key: null, // a copy must not silently steal an event mapping
      subject: src.subject,
      body: src.body,
...
export async function recordTemplateUsage(id: number): Promise<void> {
  try {
    await query(
      "UPDATE hr_email_templates SET usage_count = usage_count + 1, last_used_at = NOW() WHERE id = ?",
      [id],
    )
  } catch {
...
async function snapshotVersion(
  ...
  try {
    await query(
      `INSERT INTO hr_email_template_versions
...
export async function listVersions(templateId: number) {
  ...
  return query<any[]>(
    `SELECT id, version, name, subject, status, changed_by, created_at
     FROM hr_email_template_versions WHERE template_id = ? ORDER BY version DESC`,
    [templateId],
  )
}
