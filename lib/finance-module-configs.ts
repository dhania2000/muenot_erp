...
// Phase 8 — the five relationship types a related party can hold. Each maps to
// an existing master (never a duplicate): Customer / Vendor / Group come from
// the customers_vendors master, Employee / Director come from HR employees.
const RELATED_PARTY_RELATIONSHIPS = ["Customer", "Vendor", "Employee", "Director / Promoter", "Group / Associate"]
...
const relatedParties: ModuleConfig = {
  ...
  idPrefix: "RP",
  statusColumn: "status",
  searchColumns: ["party_id", "party_name", "relationship", "pan", "gstin", "source_id", "contact_person"],
  filters: [
  ...
  // Phase 8 — the party is always linked to an existing master (never duplicated).
  // Choosing a relationship reveals the matching picker, which fills party_name,
  // source_id + source_type and copies PAN / GSTIN from the source master.
  lookups: [
  {
  key: "customer_pick",
  label: "Select customer",
  path: "/api/finance/related-parties/lookups?source=customer",
  sourceIdColumn: "source_id",
  sourceNameColumn: "party_name",
  sourceSubColumn: "sub",
  idField: "source_id",
  nameField: "party_name",
  autofill: { source_type: "source_type", pan: "pan", gstin: "gstin" },
  visibleWhen: { field: "relationship", in: ["Customer"] },
  placeholder: "Search customers…",
  },
  {
  key: "vendor_pick",
  label: "Select vendor",
  path: "/api/finance/related-parties/lookups?source=vendor",
  sourceIdColumn: "source_id",
  sourceNameColumn: "party_name",
  sourceSubColumn: "sub",
  idField: "source_id",
  nameField: "party_name",
  autofill: { source_type: "source_type", pan: "pan", gstin: "gstin" },
  visibleWhen: { field: "relationship", in: ["Vendor"] },
  placeholder: "Search vendors…",
  },
  {
  key: "employee_pick",
  label: "Select employee",
  path: "/api/finance/related-parties/lookups?source=employee",
  sourceIdColumn: "source_id",
  sourceNameColumn: "party_name",
  sourceSubColumn: "sub",
  idField: "source_id",
  nameField: "party_name",
  autofill: { source_type: "source_type", pan: "pan", gstin: "gstin" },
  visibleWhen: { field: "relationship", in: ["Employee", "Director / Promoter"] },
  placeholder: "Search employees…",
  },
  {
  key: "group_pick",
  label: "Select group / associate",
  path: "/api/finance/related-parties/lookups?source=group",
  sourceIdColumn: "source_id",
  sourceNameColumn: "party_name",
  sourceSubColumn: "sub",
  idField: "source_id",
  nameField: "party_name",
  autofill: { source_type: "source_type", pan: "pan", gstin: "gstin" },
  visibleWhen: { field: "relationship", in: ["Group / Associate"] },
  placeholder: "Search group companies / associates…",
  },
  ],
  fields: [
  fld("Party information", "relationship", "Relationship type", "select", { options: RELATED_PARTY_RELATIONSHIPS, required: true }),
  fld("Party information", "party_name", "Party", "text", { required: true, placeholder: "Choose from the picker above" }),
  fld("Party information", "source_id", "Linked master ID", "text", { hidden: true }),
  fld("Party information", "source_type", "Source", "text", { hidden: true }),
  ...
  fld("Effective period", "effective_to", "Effective to", "date", { optional: true }),
  fld("Identity", "pan", "PAN", "text", { placeholder: "Auto-filled from master when available" }),
  fld("Identity", "gstin", "GSTIN", "text", { placeholder: "Auto-filled from master when available" }),
  ...
  { key: "source_id", label: "Linked ID", mono: true },
