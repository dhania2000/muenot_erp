...
export function EmailAttachmentPicker({ value, onChange }: { value: EmailAttachment | null; onChange: (value: EmailAttachment | null) => void }) {
  ...
  return <div className="space-y-2"><div className="flex items-center gap-2"><label className="inline-flex cursor-pointer items-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-muted"><Paperclip className="size-4" /> {loading ? "Uploading..." : "Add attachment"}<input type="file" className="sr-only" accept=".pdf,.png,.jpg,.jpeg,.webp,.gif,.doc,.docx,.xls,.xlsx" disabled={loading} onChange={e => upload(e.target.files?.[0])} /></label>{value && <span className="flex items-center gap-1 text-sm text-muted-foreground">{value.filename}<Button type="button" variant="ghost" size="icon-xs" onClick={() => onChange(null)} aria-label="Remove attachment"><X className="size-3" /></Button></span>}</div>{error && <p className="text-xs text-destructive">{error}</p>}</div>
}
