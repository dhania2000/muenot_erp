...
export function MarketingSocialClient() {
  ...
  React.useEffect(() => {
    ...
    if (status === "connected") toast.success(`${platformName} account connected`)
      ...
      toast.error(`${platformName} connection failed: token exchange failed.`)
    else if (status === "profilefail")
      toast.error(`${platformName} connection failed: could not load your profile.`)
    else if (status === "dbsave")
      toast.error(`${platformName} connection failed: could not save the account.`)
