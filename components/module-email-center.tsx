...
export function ModuleEmailCenter({ module, mode = "emails" }: { module: "finance" | "operations"; mode?: "emails" | "templates" }) {
  ...
  const [form, setForm] = useState({ name: "", to: "", subject: "", body: "" })
  const { data, mutate } = useSWR(`/api/${module}/${isTemplates ? "email-templates" : "emails"}`, fetcher)

  const submit = async () => {
    const payload = isTemplates ? { name: form.name, subject: form.subject, body: form.body } : { to: form.to, subject: form.subject, body: form.body }
  ...
  return <main className="space-y-6 p-6">
    ...
    <section className="grid gap-3 rounded-xl border p-4">
      ...
      <textarea className="min-h-32 rounded border bg-background p-2" placeholder="Email body" value={form.body} onChange={e => setForm({ ...form, body: e.target.value })} />
