...
export async function POST(request: Request) {
  ...
  // A backup package bundles every dataset of the tenant at once, so it needs
  // tenant-owner authority (an impersonating operator is capped at admin).
  const guard = isBackupPackageFormat(format) ? await requireTenantOwner() : await requireTenantAdmin()
  if (!guard.ok) return NextResponse.json({ error: guard.reason }, { status: guard.status })
  const tenantId = effectiveTenantId(guard.ctx)
  const actor = {
  ...
  try {
    assertFormatScope(format, datasetKey)
  } catch (err) {
    return NextResponse.json({ error: (err as Error).message }, { status: 400 })
  }
  try {
