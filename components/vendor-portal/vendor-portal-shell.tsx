...
import { VENDOR_PORTAL_RESOURCE_META, type VendorPortalResource } from "@/lib/vendor-portal/config"
import {
  ClipboardList,
  ReceiptText,
  CreditCard,
...
const RESOURCE_ICON: Record<VendorPortalResource, React.ComponentType<{ className?: string }>> = {
...
type NavProps = {
  resources: VendorPortalResource[]
  user: { name: string; email: string }
  vendorName: string | null
}

function NavLinks({ resources, onNavigate }: { resources: VendorPortalResource[]; onNavigate?: () => void }) {
  const pathname = usePathname()
  const items = [
    { href: "/vendor-portal", label: "Dashboard", Icon: LayoutDashboard, active: pathname === "/vendor-portal" },
    ...resources.map((r) => ({
      href: `/vendor-portal/${r}`,
      label: VENDOR_PORTAL_RESOURCE_META[r].labelPlural,
      Icon: RESOURCE_ICON[r],
      active: pathname === `/vendor-portal/${r}` || pathname.startsWith(`/vendor-portal/${r}/`),
    })),
  ]
  return (
    <nav className="flex flex-col gap-1" aria-label="Vendor portal sections">
...
export function VendorPortalShell({
  ...
  const brand = (
    <div className="flex flex-col">
      <span className="text-sm font-semibold leading-tight">{vendorName ?? "Vendor portal"}</span>
        ...
        <header className="flex h-16 items-center justify-between gap-3 border-b border-border bg-background px-4 md:hidden">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="left" className="w-72 p-0">
              <SheetTitle className="sr-only">Vendor portal navigation</SheetTitle>
              <div className="flex h-16 items-center border-b border-border px-4">{brand}</div>
              <div className="p-3">
