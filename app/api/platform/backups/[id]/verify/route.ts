...
import { type BackupActor, verifyBackupRun } from "@/lib/backup/store"
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const run = await verifyBackupRun(tenantId, runId, actor)
  if (!run) return NextResponse.json({ error: "Backup run not found" }, { status: 404 })
