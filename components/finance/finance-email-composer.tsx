...
export function FinanceEmailComposer({ onSent }: { onSent: () => void }) {
  const { data: recipData } = useSWR<{ recipients: Recipient[] }>(
    "/api/finance/emails/recipients",
    fetcher,
  )
  const { data: tplData } = useSWR<{ templates: Template[] }>(
    "/api/finance/email-templates",
    fetcher,
  )

  const [recipientMode, setRecipientMode] = useState<"parties" | "manual">("parties")
  ...
  async function submit(mode: "send" | "draft" | "schedule") {
    if (recipientMode === "parties" && selected.length === 0) {
    ...
    if (recipientMode === "parties") {
      if (selected.length > 1) payload.party_ids = selected.map((s) => s.id)
      else payload.party_id = selected[0].id
    } else {
    ...
    setBusy(mode)
    try {
      const res = await fetch("/api/finance/emails", {
        method: "POST",
  ...
  const sensitive = SENSITIVE_FINANCE_EMAIL_CATEGORIES.has(form.category)
    ...
    <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      <div className="space-y-5 rounded-xl border bg-card p-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex rounded-md border p-0.5 text-xs">
              {(["parties", "manual"] as const).map((m) => (
                ...
                >
                  {m === "parties" ? "Customers / Vendors" : "Manual email"}
                </button>
              ))}
            </div>
          </div>

          {recipientMode === "parties" ? (
            <div className="space-y-2">
              {selected.length > 0 && (
                  ...
                  {selected.map((r) => (
                    <Badge key={r.id} variant="secondary" className="gap-1">
                      ...
                      <button
                        ...
                        aria-label={`Remove ${r.party_name}`}
                      >
                        ×
                      </button>
                    </Badge>
              ...
              <Input
                placeholder="Search by name, party ID, email or type"
                value={search}
              ...
              <p className="text-xs text-muted-foreground">
                Selecting multiple parties sends a personalized copy to each. Use{" "}
                {"{{party_name}}"}, {"{{gstin}}"} etc. in the body.
              </p>
            </div>
          ) : (
        ...
        {sensitive && (
          <p className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-300">
            This is a sensitive category. Sending requires the &quot;Send Confidential Finance
            Email&quot; permission.
          </p>
        )}
        ...
        <div className="space-y-1.5">
          ...
          {showPreview ? (
            ...
            <Textarea
              className="min-h-40"
              placeholder="Write your message. Use {{party_name}} to personalize."
              value={form.body}
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border bg-muted/40 p-4 text-xs text-muted-foreground">
          ...
          <p>
            Available variables: {"{{party_name}}"}, {"{{first_name}}"}, {"{{party_id}}"},{" "}
            {"{{legal_name}}"}, {"{{contact_person}}"}, {"{{gstin}}"}, {"{{pan}}"}, {"{{email}}"}.
          </p>
        </div>
      </div>
    </div>
