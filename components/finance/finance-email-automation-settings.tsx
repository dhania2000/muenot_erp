...
export function FinanceEmailAutomationSettings() {
  const { data, mutate, isLoading } = useSWR<{ events: EventConfig[]; canManage: boolean }>(
    "/api/finance/emails/automations",
    fetcher,
  )
    ...
    mutate(
      (prev) =>
        prev
          ? {
              ...prev,
              events: prev.events.map((e) => (e.event_key === key ? { ...e, ...body } : e)),
            }
          : prev,
      false,
    )
    try {
      const res = await fetch("/api/finance/emails/automations", {
        method: "PATCH",
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          Control which finance workflow events send an automated email, the template they use, and
          whether the accounts team is CC&apos;d.
        </p>
        {!canManage && (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-3 md:justify-end">
                  ...
                  <label className="flex items-center gap-2 text-xs text-muted-foreground">
                    <Toggle
                      checked={ev.cc_accounts}
                      disabled={!canManage || !ev.enabled || savingKey === ev.event_key}
                      onChange={(next) => patch(ev.event_key, { cc_accounts: next })}
                      label={`CC accounts for ${ev.label}`}
                    />
                    CC accounts
                  </label>
                </div>
