...
export function EmailHubComposer({
  config,
  onSent,
}: {
  ...
  const { data: recipData } = useSWR<{ recipients: Recipient[] }>(
    `${config.apiBase}/recipients`,
    fetcher,
  )
  const { data: tplData } = useSWR<{ templates: Template[] }>(config.templatesUrl, fetcher)

  const [recipientMode, setRecipientMode] = useState<"directory" | "manual">("directory")
  ...
  const sensitiveSet = useMemo(
    () => new Set(config.sensitiveCategories),
    [config.sensitiveCategories],
  )
  ...
  async function submit(mode: "send" | "draft" | "schedule") {
    if (recipientMode === "directory" && selected.length === 0) {
      toast.error(`Select at least one ${config.recipientNoun}`)
      return
    }
    ...
    if (recipientMode === "directory") {
      if (selected.length > 1) payload.recipient_ids = selected.map((s) => s.id)
      else payload.recipient_id = selected[0].id
    } else {
    ...
    try {
      ...
      if (json.ok === false && json.failed) {
        throw new Error(json.errors?.[0] || `${json.failed} email(s) failed`)
      }
      const label =
        mode === "send" ? "Email sent" : mode === "draft" ? "Draft saved" : "Email scheduled"
  ...
  const sensitive = sensitiveSet.has(form.category)
    ...
    <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      <div className="space-y-5 rounded-xl border bg-card p-5">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex rounded-md border p-0.5 text-xs">
              {(["directory", "manual"] as const).map((m) => (
                ...
                >
                  {m === "directory" ? config.recipientLabelPlural : "Manual email"}
                </button>
              ))}
            </div>
          </div>
          ...
          {recipientMode === "directory" ? (
            <div className="space-y-2">
              ...
              <Input
                placeholder={config.recipientSearchPlaceholder}
                value={search}
              ...
              <p className="text-xs text-muted-foreground">
                Selecting multiple recipients sends a personalized copy to each. Use{" "}
                {`{{${config.personalization[0]}}}`} etc. in the body.
              </p>
            </div>
          ) : (
        ...
        {sensitive && (
          <p className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-300">
            This is a sensitive category. Handle its contents with care before sending.
          </p>
        )}
        ...
        <div className="space-y-1.5">
          ...
          {showPreview ? (
            ...
            <Textarea
              className="min-h-40"
              placeholder={`Write your message. Use {{${config.personalization[0]}}} to personalize.`}
              value={form.body}
      ...
      <div className="space-y-4">
        <div className="space-y-3 rounded-xl border bg-card p-5">
          ...
          <Button className="w-full" disabled={busy !== null} onClick={() => submit("send")}>
        ...
        <div className="rounded-xl border bg-muted/40 p-4 text-xs text-muted-foreground">
          ...
          <p>
            Available variables:{" "}
            {config.personalization.map((v) => `{{${v}}}`).join(", ")}.
          </p>
        </div>
      </div>
    </div>
