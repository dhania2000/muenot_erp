...
export async function GET(req:NextRequest){const s=await getSession();if(!s)return NextResponse.json({error:"Unauthorized"},{status:401});const kind=req.nextUrl.searchParams.get("kind")||"";const c=configs[kind];if(!c)return NextResponse.json({error:"Invalid kind"},{status:400});
  // Awards & appreciations drive certificate PDF / email actions, so surface the
  // employee's name + best email alongside the record via a join on hr_employees.id.
  if(kind==="awards"||kind==="appreciations")return NextResponse.json({rows:await query(`SELECT t.*, e.employee_name AS employee_name, COALESCE(e.official_email,e.personal_email) AS employee_email FROM ${c.table} t LEFT JOIN hr_employees e ON e.id=t.employee_id ORDER BY t.${c.id} DESC`)});
  return NextResponse.json({rows:await query(`SELECT * FROM ${c.table} ORDER BY ${c.id} DESC`)})}
