...
export function SalesEmailAnalytics() {
  const [days, setDays] = useState("30")
  const { data, isLoading } = useSWR<Analytics>(
    `/api/sales/emails/analytics?days=${days}`,
    fetcher,
  ...
  const headline: { label: string; value: string; hint?: string; accent?: string }[] = f
    ? [
        { label: "Emails", value: String(f.total), hint: "in range" },
        {
          label: "Delivery rate",
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-2">
        <p className="text-sm text-muted-foreground">
          Delivery and engagement across all sales mail.
        </p>
              ...
              <div className="mt-4 flex flex-wrap gap-2 border-t pt-4">
                {data!.byStatus.map((s) => (
                  <div key={s.status} className="flex items-center gap-1.5 text-xs">
                    <SalesEmailStatusBadge status={s.status} />
