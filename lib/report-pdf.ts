...
async function buildReportPdfDoc(payload: ReportExportPayload) {
  ...
  autoTable(doc, {
    ...
    didDrawPage: (data) => {
      ...
      const contact = [
        company?.taxNumber ? `${company.taxLabel || "Tax"}: ${company.taxNumber}` : "",
        company?.email,
        company?.phone,
        company?.website,
      ]
      ...
      // Period / filters band.
      const bandBits = [subtitle, ...(filterLabels ?? [])].filter(Boolean)
      ...
      doc.setTextColor(...MUTED)
      doc.text(
        reportDescription ? reportDescription : "Computer-generated report",
        marginX,
        pageHeight - 8,
      )
      doc.text(`Page ${pageNo} of ${totalPagesExp}`, pageWidth - marginX, pageHeight - 8, {
        align: "right",
      })
    },
  })
