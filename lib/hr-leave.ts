...
async function ensureQuotaLedgerColumns() {
  // Create the base ledger table if it was never provisioned. Without this,
  // every addColumn/UPDATE/index call below fails on databases that skipped the
  // original hr_leave_quota_history migration. No FK to hr_employees here — the
  // rest of the leave engine treats hr_employees as the source of truth and
  // deliberately drops that phantom relationship.
  await query(
