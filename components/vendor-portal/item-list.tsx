...
/** Map a payment/compliance/invoice status to a badge tone. */
function statusVariant(status: string | null): "default" | "secondary" | "destructive" | "outline" {
  const s = (status ?? "").toLowerCase()
  if (["paid", "approved", "compliant", "verified", "settled", "cleared"].some((k) => s.includes(k))) return "default"
  if (["overdue", "rejected", "expired", "non-compliant", "failed"].some((k) => s.includes(k))) return "destructive"
  if (["submitted", "pending", "processing", "in review", "partial"].some((k) => s.includes(k))) return "secondary"
  return "outline"
}
