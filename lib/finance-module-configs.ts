...
const customersVendors: ModuleConfig = {
  ...
  fields: [
    ...
    fld("Identity", "party_category", "Vendor category", "select", { options: VENDOR_CATEGORIES, optional: true, emptyLabel: "— Select category —" }),
    ...
    fld("Registered address", "billing_address", "Registered address", "textarea"),
    fld("Registered address", "city", "City", "text"),
    fld("Registered address", "state", "State", "text"),
    fld("Registered address", "state_code", "State code", "text"),
    fld("Registered address", "pin_code", "PIN code", "text"),
    fld("Registered address", "country", "Country", "text"),
    fld("Tax profile", "gstin", "GSTIN", "text"),
    fld("Tax profile", "pan", "PAN", "text"),
    fld("Tax profile", "tan", "TAN", "text"),
    fld("Tax profile", "gst_registration_type", "GST registration type", "select", { options: ["Regular", "Composition", "Unregistered", "SEZ", "Overseas"], optional: true, emptyLabel: "— Select —" }),
    fld("Tax profile", "tds_applicable", "TDS applicable", "checkbox"),
    fld("Tax profile", "tds_section", "TDS section", "text"),
    fld("Tax profile", "tds_rate", "TDS rate %", "number"),
    ...
    // GST snapshot — server-populated by GSTIN verification, never hand-edited.
    fld("GST verification", "gst_trade_name", "Trade name", "text", { hidden: true }),
    fld("GST verification", "gst_status", "GST status", "text", { hidden: true }),
    ...
    fld("GST verification", "gst_block_status", "e-Invoice / block status", "text", { hidden: true }),
    ...
    fld("Status", "status", "Vendor status", "select", { options: ["Active", "Inactive", "Archived"] }),
    fld("Status", "notes", "Notes", "textarea"),
  ],
  tableColumns: [
    { key: "party_id", label: "Vendor ID", mono: true },
    { key: "customer_name", label: "Vendor", sub: "party_category" },
    { key: "gstin", label: "GSTIN", mono: true },
    { key: "gst_verification_status", label: "GST", badge: { Verified: "default", Cancelled: "destructive", Suspended: "destructive", Invalid: "destructive", Unverified: "outline" } },
    { key: "city", label: "City", sub: "state" },
    { key: "status", label: "Status", badge: { Active: "default", Inactive: "outline", Archived: "secondary" } },
  ],
  kpis: [
    { label: "Total Vendors", key: "total_rows", icon: "Users" },
    { label: "GST Verified", key: "verified_rows", icon: "TrendingUp" },
    { label: "Credit Limit", key: "total_credit", money: true, icon: "Coins" },
  ],
  summarySelect: "COUNT(*) total_rows, COALESCE(SUM(CASE WHEN gst_verification_status = 'Verified' THEN 1 ELSE 0 END),0) verified_rows, COALESCE(SUM(credit_limit),0) total_credit",
}
