...
export function CallDialer({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await fetch("/api/sales/calls/token", { method: "POST" })
  ...
  // Keep a ref of seconds so finishCall (bound in event handlers) reads the latest value.
  ...
  async function saveOutcome() {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/sales/calls/${callId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{target?.name ? `Call ${target.name}` : "Call lead"}</DialogTitle>
