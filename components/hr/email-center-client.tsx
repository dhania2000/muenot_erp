...
export function HrEmailCenter({ mode = "emails" }: { mode?: "emails" | "templates" }) {
  ...
  const [form, setForm] = useState({ name: "", to_email: "", to_name: "", subject: "", body: "" })
  const [message, setMessage] = useState("")
  async function submit(e: React.FormEvent) {
    e.preventDefault()
    const endpoint = mode === "emails" ? "/api/hr/emails" : "/api/hr/email-templates"
    const payload = mode === "emails" ? form : { name: form.name, subject: form.subject, body: form.body }
    const res = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
    const json = await res.json(); setMessage(json.error || "Saved successfully"); if (res.ok) { setForm({ name: "", to_email: "", to_name: "", subject: "", body: "" }); mutate() }
  }
