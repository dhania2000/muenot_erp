...
/**
 * Aggregated WhatsApp analytics for a single tenant. The tenant id is NEVER
 * accepted from client input: it defaults to the current authenticated tenant
 * (derived from session context) and may only be overridden internally by a
 * trusted platform/background caller that has already established the tenant.
 * Every query below carries an explicit `tenant_id = ?` predicate (including
 * inside subqueries and joins) so aggregates can never span tenants.
 */
export async function getWhatsAppAnalytics(tenantId: number = currentTenantId()): Promise<WhatsAppAnalytics> {
  ...
  const [
    ...
    query<{ user_id: number; name: string; open_count: number; closed7d: number; sent7d: number }[]>(
      `SELECT u.id AS user_id, u.name,
              (SELECT COUNT(*) FROM \`marketing_whatsapp_conversations\` c WHERE c.assigned_agent_id = u.id AND c.tenant_id = ? AND c.status <> 'closed') AS open_count,
         ...
         FROM \`users\` u
         JOIN \`marketing_whatsapp_agent_settings\` s ON s.user_id = u.id AND s.tenant_id = u.tenant_id AND s.is_agent = 1
        WHERE u.tenant_id = ?
